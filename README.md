...
Select **Next** 2 times, and give permissions to the installer.

At "Choose Components," make sure all check boxes are selected 
(should be default)
...
Double-click `kicad-unified-universal-#.#.#.dmg` file in finder, 
then click and drag the `KiCad.app` to Application folder.

Then eject the `.dmg` file from Finder.
</details>
...
> Xwayland for GTK3. Installing via PPAs defaults to Xwayland, likely bringing
> graphical bugs.
</details>
...
Install **Git for Windows** from this link: [https://git-scm.com/install/windows](https://git-scm.com/install/windows).
![Installation link](img/git-windows0.png)

After installing the executable, give the installer permissions.
![Give permission for installer](img/git-windows1.png)

Select **Next** 4 times, and reach "Choosing the default editor used by Git"
...
If you perfer installing ONLY `git` with `homebrew`, here is the command:
```sh
brew install git
```
</details>
